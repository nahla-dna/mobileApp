...
def main(page: ft.Page):
    ...
    def apply_filters():
        ...
        if location_filter.value:
            filtered = [v for v in filtered if location_filter.value.lower() in v.get("location", "").lower()]
    ...
    # ---------------- NEARBY VILLAS ----------------
    def load_nearby_villas():
        show_message("📍 Detecting your location...")

        def fetch():
            try:
                g = geocoder.ip('me')
                if not g.ok:
                    show_message("❌ Could not detect location.")
                    return

                lat, lon = g.latlng
                show_message(f"📍 Found: {round(lat, 4)}, {round(lon, 4)}")

                res = requests.get(BASE_URL + "villas/nearby/", params={
                    "lat": lat,
                    "lon": lon,
                    "radius": 1
                })
                ...
                nearby = res.json()
                print("NEARBY RAW:", nearby)

                
                if isinstance(nearby, dict):
                    nearby = nearby.get("results") or nearby.get("villas") or []

                if not nearby:
                    show_message("No villas found near you within 1km.")
                    return

                
                for v in nearby:
                    if "distance_km" in v:
                        v["distance_label"] = f"{v['distance_km']} km away"

                display_villas(nearby, show_distance=True)

            except Exception as ex:
                print("NEARBY ERROR:", str(ex))
                show_message(f"Error: {str(ex)}")
    ...
    def load_villas():
        ...
        print("VILLAS RAW:", villas_data[:1])  

        display_villas(villas_data)

    def display_villas(villas, show_distance=False):
        cards = []
        ...
        for v in villas:
            ...
            image_url = v.get("image") or v.get("main_image") or None

            distance_widget = ft.Text(
                f"🗺️ {v.get('distance_label', '')}",
                size=12,
                color="lightblue"
            ) if show_distance and v.get("distance_label") else ft.Container()
            ...
            card = ft.Container(
                content=ft.Column([
                    ...
                    ft.Column([
                        ft.Text(v.get("name", "Unknown"), size=18, weight="bold"),
                        ft.Text(f"📍 {v.get('location', 'N/A')}", size=13, color="grey"),
                        ft.Text(f"💰 Rs {v.get('price_per_night', '0')}", size=14, weight="bold"),
                        distance_widget,
                    ], spacing=3),
        ...
        page.add(
            ...
            ft.Row([
                ft.ElevatedButton(
                    "📍 Near Me",
                    on_click=lambda e: load_nearby_villas(),
                    bgcolor="teal",
                    color="white"
                ),
                ft.TextButton("Show All", on_click=lambda e: load_villas()),
            ]),
